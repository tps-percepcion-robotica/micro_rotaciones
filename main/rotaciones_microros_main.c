...
static rcl_publisher_t rotacion_publisher;

static rcl_subscription_t vector_subscriber;
static rcl_subscription_t euler_subscriber;

static std_msgs__msg__ euler_angles;
static std_msgs__msg__ vector_rotado;
static std_msgs__msg__ vector_original;


// TODO : Implementar una funcion que muestre el valor del vector original y el vector rotado en la terminal, para poder verificar que la rotacion se esta realizando correctamente.

// TODO : Implementar una funcion que reciba los datos de los topicos /vector y /euler y realice la rotacion del vector original, para luego publicar el vector rotado en el topico /vector_rotado usando microrotaciones.
...
static void timer_callback(rcl_timer_t *timer, int64_t last_call_time){
    ...
    RCSOFTCHECK(rcl_publish(&rotacion_publisher, &posicion_msg, NULL));
    
    ESP_LOGI(TAG, "Vector original: (%.2f, %.2f, %.2f)", vector_original.x, vector_original.y, vector_original.z);
    ESP_LOGI(TAG, "Vector rotado: (%.2f, %.2f, %.2f)", vector_rotado.x, vector_rotado.y, vector_rotado.z);
}
}

// TODO : Configurar el publicador del topico /vector_original
...
static void micro_ros_task(void *arg){
    while (1) {
        ...
        RCCHECK(rclc_publisher_init_default(
            &rotacion_publisher, &node,
            ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, ),  // TODO :  Definir tipo de dato
            "vector_rotado"));
        ...
        rcl_publisher_fini(&rotacion_publisher, &node);
